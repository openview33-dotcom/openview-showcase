...
const textTestimonials = [
  { text: "Além da qualidade, o atendimento é impecável. A equipe é próxima, entende o negócio e entrega sempre além do esperado.", author: "Cliente OpenView" },
  { text: "Logo no primeiro mês já sentimos a diferença. As vendas aumentaram, o movimento cresceu e muitos clientes novos começaram a chegar na loja. Valeu cada centavo do investimento!", author: "Cliente OpenView" },
  { text: "Nosso negócio mudou completamente depois que começamos com a OpenView. Os clientes comentam, chegam pelo Instagram e elogiam a comunicação. Foi a melhor decisão que tomamos.", author: "Cliente OpenView" },
  { text: "Entraram muitos clientes novos! Percebemos que vieram do Google e do Instagram.", author: "Cliente OpenView" },
  { text: "Fiquei muito feliz com o resultado. É um salto de qualidade gigantesco. Parabenizo toda a equipe, estou certo de que fiz uma ótima escolha.", author: "Cliente OpenView" },
...
const CasesSection = () => {
  ...
  return (
    <section id="cases" className="py-32 section-padding section-light">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="font-body text-sm tracking-[0.3em] uppercase text-yellow mb-4">
            ◆ Cases
          </p>
            ...
            <button
              ...
              className={`px-8 py-3 font-body text-sm tracking-wider uppercase transition-all rounded-full ${
                activeTab === tab
                  ...
                  : "bg-transparent text-muted-foreground border border-muted-foreground/30 hover:border-foreground hover:text-foreground"
              }`}
            >
            ...
            {videoTestimonials.map((video) => (
              <div key={video.videoId} className="group relative aspect-[9/16] rounded-2xl overflow-hidden bg-muted cursor-pointer" onClick={() => setPlayingVideo(video.videoId)}>
                {playingVideo === video.videoId ? (
                  ...
                  <>
                    <img src={video.thumb} alt={`Depoimento de ${video.name}`} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <p className="font-body text-sm tracking-wider uppercase text-white">{video.name}</p>
                    </div>
                  </>
                )}
              </div>
